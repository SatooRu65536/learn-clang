...
int main(void) {
  ...
  for (int i = 0; i < learn_length; i++) {
    // sizeof(Iris) * learn_length
    unsigned long size = sizeof(Iris) * (i + 1);
    if ((array_learn = (Iris *)realloc(array_learn, size)) == NULL) {
      printf("No enough memory.\n");
      exit(1);
    }
  ...
  for (int i = 0; i < test_length; i++) {
    unsigned long size = sizeof(Iris) * (test_length);
    if ((array_test = (Iris *)realloc(array_test, size)) == NULL) {
      printf("No enough memory.\n");
      exit(1);
    }
